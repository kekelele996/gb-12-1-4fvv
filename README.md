...
- **申请递交锁定**：顾问递交时系统校验必交材料全部完成、个人陈述（PS）当前版本无未解决批注、申请批次未截止；任一不满足则拒绝递交并逐项列出阻塞项
- 递交成功后状态一次性转为「已提交」，同时固化材料清单与个人陈述版本快照；此后新增材料或文书新版本均不影响快照，已固化材料和申请批次不可移除或更换
- 管理员说明原因可回退递交（材料/批次解锁），回退后旧快照保留可查，可重新递交生成新快照；重复或并发递交只生成一个快照
...
│   ├── materials/                    # 材料清单模块
│   ├── submissions/                  # 申请递交锁定模块（快照、阻塞项校验、回退）
│   ├── messages/                     # 站内消息模块
│   ├── timelines/                    # 时间线模块（通知）
